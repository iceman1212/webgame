...
import { GAME_CONFIG, ASSETS_CONFIG, COLORS } from '../core/gameConfig.js';
...
export function spawnBalloons(currentQuestion: Question | null, existingBalloons: Balloon[]): Balloon[] {
  return GameErrorHandler.safeExecute(() => {
    ...
    while (choices.length < GAME_CONFIG.BALLOON_COUNT) {
      ...
      let fake: number = correctAns + (Math.floor(Math.random() * range) - GAME_CONFIG.DISTRACTOR_RANGE);
    ...
    return choices.map((val, idx): Balloon => {
      ...
      const xPos: number = idx * (canvasWidth / GAME_CONFIG.BALLOON_COUNT) + (canvasWidth / (GAME_CONFIG.BALLOON_COUNT * 2)) - BALLOON_RADIUS;
      // 随机选择一个气球图片
      ...
      return {
        ...
        speed: GAME_CONFIG.BALLOON_SPEED_MIN + Math.random() * (GAME_CONFIG.BALLOON_SPEED_MAX - GAME_CONFIG.BALLOON_SPEED_MIN), // 使用配置的速度范围
        floatOffset: Math.random() * Math.PI * 2 // 气球漂浮动画的随机相位偏移
      };
    });
  }, 'spawning balloons', existingBalloons) || existingBalloons;
}
...
export function drawBalloons(balloons: Balloon[]): void {
  GameErrorHandler.safeExecute(() => {
    ...
    // 绘制气球阴影
    context.beginPath();
    context.arc(bx + BALLOON_RADIUS + GAME_CONFIG.SHADOW_OFFSET, by + BALLOON_RADIUS + GAME_CONFIG.SHADOW_OFFSET, BALLOON_RADIUS, 0, Math.PI * 2);
