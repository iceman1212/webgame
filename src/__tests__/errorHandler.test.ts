...
describe('GameErrorHandler', () => {
  ...
  describe('handleResourceLoadError', () => {
    it('应该记录资源加载错误', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('Failed to load image: /path/to/image.png', error);
    });
  ...
  describe('handleGameStateError', () => {
    it('应该记录游戏状态错误', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Game state error during update:', error);
    });
  });
  ...
  describe('handleCanvasError', () => {
    it('应该记录Canvas错误', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Canvas error during rendering:', error);
    });
  });
  ...
  describe('safeExecute', () => {
    ...
    it('应该捕获错误并返回回退值', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error in test operation:', expect.any(Error));
  ...
  describe('safeExecuteAsync', () => {
    ...
    it('应该捕获异步错误并返回回退值', async () => {
      ...
      const result = await GameErrorHandler.safeExecuteAsync(
        testFn,
        'async test',
        'async fallback'
      );

      expect(console.error).toHaveBeenCalledWith('Async error in async test:', expect.any(Error));
  ...
  describe('validateRequiredElements', () => {
    ...
    it('应该在有元素缺失时返回false', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Missing required DOM elements:', ['button']);
    });
    ...
    it('应该记录所有缺失的元素', () => {
      ...
      const result = GameErrorHandler.validateRequiredElements(elements);
      expect(result).toBe(false);
      expect(console.error).toHaveBeenCalledWith('Missing required DOM elements:', [
        'canvas',
        'button',
      ]);
    });
