import { ctx } from '../utils/domElements.js';
...
function drawFallbackPlayer(context: CanvasRenderingContext2D, player: Player): void {
  ...
  for (let i = 0; i < 5; i++) {
    context.lineTo(
      (Math.cos(((18 + i * 72) / 180) * Math.PI) * player.size) / 2,
      (-Math.sin(((18 + i * 72) / 180) * Math.PI) * player.size) / 2
    );
    context.lineTo(
      (Math.cos(((54 + i * 72) / 180) * Math.PI) * player.size) / 4,
      (-Math.sin(((54 + i * 72) / 180) * Math.PI) * player.size) / 4
    );
  }
  // 关闭路径
  context.closePath();
